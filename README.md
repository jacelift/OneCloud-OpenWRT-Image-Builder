...
* `etc/config/network`：网络接口配置，默认IP为`192.168.1.1`，网关`192.168.1.1`，DNS`114.114.114.114`。或者也可以将`network_set-lan-as-dhcp-client`文件替换成`network`，实现网络接口LAN配置成DHCP client协议。
